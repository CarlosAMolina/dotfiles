...
-- NOTE: `Plug` is a Vimscript command defined by vim-plug (not a public Lua
-- API), so it must be driven through vim.cmd.
vim.cmd([[
  call plug#begin()
  " coc
...
-- I have this problem in mac: https://stackoverflow.com/questions/65160481/neovim-on-windows-cant-find-python-provider
...
vim.keymap.set('n', 'gde', '<Plug>(coc-definition)', { silent = true })
...
if vim.fn.has('nvim-0.4.0') == 1 or vim.fn.has('patch-8.2.0750') == 1 then
  ...
  vim.keymap.set('i', '<C-f>', function()
    return vim.fn['coc#float#has_scroll']() == 1 and vim.fn.eval('coc#float#scroll(1)') or '<Right>'
  end, { silent = true, nowait = true, expr = true })
  vim.keymap.set('i', '<C-b>', function()
    return vim.fn['coc#float#has_scroll']() == 1 and vim.fn.eval('coc#float#scroll(0)') or '<Left>'
  end, { silent = true, nowait = true, expr = true })
...
vim.keymap.set('n', '<leader>gdi', ':Gvdiffsplit!<CR>')
